...
from agentprof.report.runner import REPORT_ID_MAX_LENGTH, _html_report, generate_report
...
def test_html_report_renders_sections_and_cost_ledger(tmp_path: Path) -> None:
    store = DuckDBStore(tmp_path / "agentprof.duckdb")
    _seed_retry_issue(store)
    result = generate_report(
        store,
    ...
    assert "<title>AgentProf Report: tracer</title>" in html
    assert "<h2>Summary</h2>" in html
    assert "<h2>Issues</h2>" in html
    assert "<h2>Cost Ledger</h2>" in html
    assert "Repeated failing call to refund_policy_lookup" in html
    assert "Stop retrying deterministic failures." in html
    assert "wasted_retry_cost" in html
    assert "$0.020000000" in html


def test_html_report_renders_empty_states(tmp_path: Path) -> None:
    store = DuckDBStore(tmp_path / "agentprof.duckdb")
    result = generate_report(
        store,
        project="tracer",
        output_dir=tmp_path / "reports",
        report_id="empty-html-render-report",
        generated_at=_dt("2026-04-26T12:00:00+00:00"),
    )
    payload = json.loads(result.report_json_path.read_text(encoding="utf-8"))
    ...
    assert "No issues have been generated yet." in html
    assert "No cost ledger entries have been generated yet." in html


def test_html_report_embeds_multi_agent_visual_reference(tmp_path: Path) -> None:
    store = DuckDBStore(tmp_path / "agentprof.duckdb")
    _seed_multi_agent_issue(store)
    result = generate_report(
        store,
        project="tracer",
        output_dir=tmp_path / "reports",
        report_id="html-multi-agent-report",
        generated_at=_dt("2026-04-26T12:00:00+00:00"),
    )
    payload = json.loads(result.report_json_path.read_text(encoding="utf-8"))
    ...
    assert "<h2>Visuals</h2>" in html
    assert 'src="html-multi-agent-report-multi-agent-waste.svg"' in html
    assert "Multi-agent waste estimate generated by AgentProf." in html


def test_html_report_escapes_trace_controlled_content(tmp_path: Path) -> None:
    store = DuckDBStore(tmp_path / "agentprof.duckdb")
    _seed_markdown_injection_issue(store)
    result = generate_report(
        store,
        project='Tracer <b title="x">Project</b>',
        output_dir=tmp_path / "reports",
        report_id="escaped-html-report",
        generated_at=_dt("2026-04-26T12:00:00+00:00"),
    )
    payload = json.loads(result.report_json_path.read_text(encoding="utf-8"))
    ...
    assert 'Tracer &lt;b title=&quot;x&quot;&gt;Project&lt;/b&gt;' in html
